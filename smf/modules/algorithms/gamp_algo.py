...
def forward_pass_parallel(
    ...
    W_sel = W_hat[:, i_idx.long(), :]  # (A, C_max, M)

    # X_sel[a, c, μ] = X_hat[a, μ, j_idx[c]]
    X_sel = X_hat[:, :, j_idx.long()].transpose(1, 2)  # (A, C_max, M)
...
def compute_variance_parallel(
    ...
    W_sel = W_hat[:, i_idx.long(), :]       # (A, C_max, M)
    X_sel = X_hat[:, :, j_idx.long()].transpose(1, 2)  # (A, C_max, M)
    W_var_sel = W_var[:, i_idx.long(), :]   # (A, C_max, M)
    X_var_sel = X_var[:, :, j_idx.long()].transpose(1, 2)
...
def bigamp_step_disjoint_union(
    ...
    W_sel = W_flat[:, i_offset.long(), :]  # (A, SC, M)
    X_sel = X_flat[:, j_offset.long(), :]  # (A, SC, M)
    W_var_sel = W_var_flat[:, i_offset.long(), :]  # (A, SC, M)
    X_var_sel = X_var_flat[:, j_offset.long(), :]  # (A, SC, M)
