#!/usr/bin/env python
"""
Parallel noise sweep for the random-graph version of Dence_Alternating G-AMP.

This fixes alpha and sweeps noise_var, writing a CSV summary and a plot with
noise on the horizontal axis and cosine similarity on the vertical axis.
"""
...
DEFAULT_M = 50
DEFAULT_ALPHA = 2.5
DEFAULT_NOISE_START = 1e-5
DEFAULT_NOISE_STOP = 0.01
DEFAULT_NOISE_STEP = 0.0005
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fix alpha and sweep noise_var for the random-graph version of "
            "Dence_Alternating G-AMP."
        )
    )
    parser.add_argument(
        "--N",
...
def aggregate_results(
    ...
    for noise_var in noise_values:
        ...
        cosine_values = [record["cosine_similarity"] for record in noise_records]
...
def save_metrics_csv(
    ...
    lines = []
    header = (
        "noise_var,alpha,num_observed,completed_replicas,cosine_similarity_mean,"
...
def plot_results(
    ...
    means = [results[noise]["cosine_similarity_mean"] for noise in completed_noises]
    stds = [results[noise]["cosine_similarity_std"] for noise in completed_noises]
    ...
    ax.set_title(
        f"Cosine Similarity vs Noise (alpha={args.alpha})\n"
    ...
    plt.tight_layout()
    plt.savefig(
        plots_dir / "cosine_similarity_vs_noise.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.close(fig)
...
def save_config(
    ...
    config = {
        "algorithm": "gamp_Dence_Alternating_random_graph_noise_sweep_parallel",
        ...
        "shared_teacher_graph_global": True,
        ...
        "output_files": [
            ...
            "results.npz",
            "progress.yaml",
            "plots/cosine_similarity_vs_noise.png",
        ],
    }
    write_text_atomic(
...
def print_summary(results: dict[float, dict[str, Any]], total_time: float) -> None:
    ...
    print("=" * 68)
    print(f"{'Noise':>12} | {'CosSim':^20} | {'Loss':^20} | {'Steps':>8}")
    print("-" * 68)
...
def main() -> int:
    ...
    results_dir = results_root / (
        f"{timestamp}_gamp_Dence_Alternating_random_graph_noise_sweep_"
        f"{args.N1}x{args.M}_alpha{args.alpha}_"
    ...
    print("=" * 72)
    print("Parallel Dence_Alternating G-AMP - Random Graph Noise Sweep")
    print("=" * 72)
    ...
    print(f"Plot saved: {results_dir / 'plots' / 'cosine_similarity_vs_noise.png'}")
