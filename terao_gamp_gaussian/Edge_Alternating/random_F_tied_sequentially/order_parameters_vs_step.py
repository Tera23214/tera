#!/usr/bin/env python
"""
Parallel order-parameter-vs-step runner for tied random-F Edge_Alternating.
"""
...
from terao_gamp_gaussian.Edge_Alternating.random_F_tied_sequentially.core import (  # noqa: E402
...
FINAL_PAIR_Q_KEYS = ["q_W", "q_X", "q_S", "q_Y"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "random-F tied-factor Edge_Alternating variant."
        )
    )
...
def save_config(results_dir: Path, args: argparse.Namespace, devices: list[str]) -> None:
    config = {
        "algorithm": "gamp_Edge_Alternating_random_F_tied_order_parameters_vs_step_sequential",
        ...
        "tied_factors": True,
        "tied_rule": "X = W.T",
        "tie_projection": "after_initialization_and_after_each_half_step",
        "N1_equals_N2_required": True,
        "algorithm_note": "heuristic tied-factor version, not exact one-variable AMP",
        "stores_F_edge": False,
        ...
        "update_scheme": "alternating_W_then_X_with_tie_projection_after_each_half_step",
        "step_definition": "one_W_update_plus_tie_projection_plus_one_X_update_plus_tie_projection",
...
def save_progress_outputs(
    ...
    if records:
        ...
        plot_order_parameter(
            plots_dir,
            steps,
            order_parameter_histories["tie_error"],
            "tie_error",
            "Tie Error",
            "tie_error_vs_step.png",
            args,
        )
...
def _worker_main(
    ...
    try:
        ...
        from terao_gamp_gaussian.Edge_Alternating.random_F_tied_sequentially.core import (
...
def main() -> int:
    ...
    if args.num_replicas < 2:
        raise ValueError("q_W, q_X, q_S, and q_Y require --num-replicas >= 2.")
    if args.N1 != args.N2:
        raise ValueError("The tied-factor model requires N1 == N2. Use --N or matching --N1/--N2.")
    ...
    print("=" * 72)
    print("Sequential Random-F Tied Edge_Alternating Order Parameters vs Step")
    print("=" * 72)
    ...
    print("Tied rule: X = W.T")
    print("Step definition: W update + tie projection + X update + tie projection")
    ...
    results_dir = results_root / (
        f"{timestamp}_order_parameters_vs_step_Edge_Alternating_random_F_tied_sequential_"
