...
informative student initialization and cosine-similarity evaluation.
...
def save_config(results_dir: Path, args: argparse.Namespace, device: torch.device) -> None:
    config = {
        "algorithm": "gamp_Dence_F_1_onsager_cosine_loss_vs_step_informative_init",
        "alpha": args.alpha,
        "N1": args.N1,
        "N2": args.N2,
        "M": args.M,
...
def plot_linear_loss(
    ...
    ax.set_title(
        f"Loss vs Step (alpha={args.alpha}, epsilon={args.init_epsilon}, "
...
def plot_log_loss(
    ...
    ax.set_title(
        f"log10(Loss) vs Step (alpha={args.alpha}, epsilon={args.init_epsilon}, "
        f"N1={args.N1}, N2={args.N2}, M={args.M}, {args.num_replicas} replicas)",
        fontsize=14,
    )
    ax.grid(True, alpha=0.3)
...
def main() -> None:
    ...
    results_dir_name = (
        f"{timestamp}_loss_vs_step_Dence_F_1_onsager_cosine_informative_epsilon{args.init_epsilon}_"
