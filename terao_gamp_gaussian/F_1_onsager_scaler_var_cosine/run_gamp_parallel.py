#!/usr/bin/env python
"""
G-AMP Parallel Simulation Runner with F=1, Onsager Correction, and
...
Usage:
    python -m terao_gamp_gaussian.F_1_onsager_scaler_var_cosine.run_gamp_parallel \\
        --num-workers 8
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--num-replicas", type=int, default=20)
...
def save_config(
    ...
    config = {
        "algorithm": "gamp_F_1_onsager_cosine_parallel",
        "N1": args.N1,
        "N2": args.N2,
        "M": args.M,
...
def plot_cosine_similarity_vs_alpha(
    plots_dir: Path,
    results: dict,
    alphas_list: list[float],
    ...
    cosine_similarity_sems = [
        std / math.sqrt(args.num_replicas) for std in cosine_similarity_stds
    ]
...
def main() -> None:
    ...
    print("=" * 60)
    print("G-AMP with F=1 + Onsager Correction (PARALLEL, Cosine Similarity)")
    print("=" * 60)
    ...
    results_dir_name = (
        f"{timestamp}_gamp_F_1_onsager_cosine_{args.N1}x{args.M}"
    ...
    for alpha in alphas:
        ...
        cosine_similarity_values = [r["cosine_similarity"] for r in alpha_results]
    ...
    plot_cosine_similarity_vs_alpha(plots_dir, results, alphas_list, args)
