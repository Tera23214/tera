#!/usr/bin/env python
"""
G-AMP Warm Start with Observation Noise.
...
SIGMA_INIT_VALUES = [float('inf'), 1.0, 0.5, 0.1, 0.0]

CONVERGENCE_THRESHOLD = 1e-6

# ============================================================================
# G-AMP Training with Noise
# ============================================================================
...
if __name__ == "__main__":
    print("=" * 60)
    print("G-AMP Warm Start with Observation Noise")
    ...
    results_dir = Path(__file__).parent / "results" / f"{timestamp}_gamp_warm_noise_sigmaY{SIGMA_Y}"
    results_dir.mkdir(parents=True, exist_ok=True)
    print(f"Results directory: {results_dir}")
    
    # Save configuration
    config = {
        'algorithm': 'gamp_warm_noise',
        'N1': N1, 'N2': N2, 'M': M,
    ...
    for sigma_init in SIGMA_INIT_VALUES:
        ...
        for alpha in alphas:
            ...
            print(f"σ_init={s_label}, α={alpha:.2f}: Q_Y = {mean_qy:.4f} ± {std_qy:.4f} [{completed}/{total_tasks}]")
    ...
    ax.set_title(f'G-AMP Warm Start with Observation Noise\n({N1}×{N2}, M={M}, σ_Y={SIGMA_Y}, {NUM_REPLICAS} replicas)', fontsize=16)
    ...
    with open(csv_path, 'w') as f:
        ...
        for s in SIGMA_INIT_VALUES:
            ...
            header += f",Q_Y_mean_sigmaInit{s_str},Q_Y_std_sigmaInit{s_str}"
        f.write(header + "\n")
