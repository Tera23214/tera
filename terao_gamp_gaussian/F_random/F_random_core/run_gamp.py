#!/usr/bin/env python
"""
G-AMP Simulation Runner with Random F.

Runs G-AMP algorithm with F ~ N(0,1) for various alpha values.
"""
...
from terao_gamp_gaussian.F_random.F_random_core.core import train_single_replica
...
if __name__ == "__main__":
    print("=" * 60)
    print("G-AMP with Random F ~ N(0,1)")
    ...
    results_dir_name = f"{timestamp}_gamp_F_random_{N1}x{M}_alpha{ALPHA_START}-{ALPHA_STOP}"
    ...
    ax.set_title(f'Phase Transition (G-AMP with Random F)\n({N1}×{N2}, M={M}, {MAX_STEPS} steps)', fontsize=16)
