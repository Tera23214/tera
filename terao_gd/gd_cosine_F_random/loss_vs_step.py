#!/usr/bin/env python
"""
Plot full-batch AGD loss vs step for the F-random observation model.

Training model:
    Y_ij = lambda / sqrt(M) * sum_mu F_ij,mu W_i,mu X_mu,j + noise

The student receives the full F and estimates W, X by alternating full-batch
AGD.
"""
...
from terao_gd.gd_cosine_F_random.gd import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot full-batch AGD loss vs step for F-random model."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Base coefficient for auto LR scaling: lr = lr_base / sqrt(N*N*M).",
    )
...
def resolve_lr(args: argparse.Namespace) -> float:
    ...
    return float(args.lr_base / math.sqrt(args.N * args.N * args.M))
...
def save_config(
    ...
    config = {
        "algorithm": "agd_cosine_F_random_loss_vs_step",
        "alpha": args.alpha,
        "N": args.N,
        "M": args.M,
        "lambda": args.lambda_,
...
def plot_loss(
    ...
    ax.set_title(
        f"F-random Full-batch AGD Loss vs Step "
...
def main() -> None:
    ...
    print("=" * 60)
    print("Loss vs Step for F-random Full-batch AGD")
    ...
    print(
        f"max_steps={args.max_steps}, lr={lr:.6e}, noise_var={args.noise_var:.6e}, "
    ...
    results_dir_name = (
        f"{timestamp}_agd_F_random_loss_vs_step_alpha{args.alpha}_"
