...
if __name__ == "__main__":
    ...
    for alpha in alphas:
        ...
        for replica_id in range(NUM_REPLICAS):
            ...
            completed += 1
            print(f"α={alpha:.2f}, replica {replica_id+1}/{NUM_REPLICAS}: Q_Y={qy:.4f}, Loss={final_loss:.2e}, Steps={steps_taken} ({dt:.1f}s) [{completed}/{total_tasks}]")
    ...
    for alpha in sorted(results.keys()):
        r = results[alpha]
        print(f"{alpha:6.2f} | {r['qy_mean']:8.4f} ± {r['qy_std']:<8.4f} | {r['loss_mean']:8.2e} ± {r['loss_std']:<8.2e} | {r['steps_mean']:8.0f}")
    ...
    with open(csv_path, 'w') as f:
        ...
        for i in range(NUM_REPLICAS):
            header += f",qy_replica_{i},loss_replica_{i}"
        f.write(header + "\n")
        ...
        for alpha in alphas_list:
            ...
            for qy_v, loss_v in zip(r['qy_values'], r['loss_values']):
                line += f",{qy_v},{loss_v}"
            f.write(line + "\n")
