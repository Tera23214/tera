...
# UI Mode Detection: When running in Streamlit/UI, disable Rich Live to avoid scrolling issues
UI_MODE = os.environ.get("SMF_UI_MODE", "0") == "1"
...
class UnifiedProgress:
    ...
    def start(self):
        ...
        # In UI mode, disable Rich Live to avoid scrolling issues
        if UI_MODE or not RICH_AVAILABLE:
            print(f"[Progress] Starting training ({self.num_alphas} alphas, {self.steps_per_alpha} steps)")
            self._live = None
            return
    ...
    def update_step(self, current: int, total: int = None):
        ...
        # In UI mode, print simple progress every 100 steps
        if UI_MODE:
            if current % 100 == 0 or current == total:
                pct = current / total * 100 if total else 0
                print(f"Step {current}/{total} ({pct:.0f}%) | α={self._current_alpha:.2f}")
            return
...
class ExperimentProgress:
    ...
    # === Methods for compatibility with runner.py ===
    
    def start_batch(self, batch_idx: int, batch_alphas: List[float]):
        """Start tracking a batch of alphas (compatibility method)."""
        if batch_alphas:
            self.start_alpha(batch_alphas[0])
    ...
    def update_step(self, current: int, total: int = None):
        ...
        if UI_MODE:
            if current % 100 == 0 or current == total:
                pct = current / total * 100 if total else 0
                print(f"Step {current}/{total} ({pct:.0f}%) | α={self.current_alpha:.2f}")
            return
        
        # ExperimentProgress doesn't have step-level tracking
        # Just update the status if we have a progress bar
        if self._progress and self._alpha_task is not None:
            status = f"α={self.current_alpha:.2f} step {current}"
            if total:
                status += f"/{total}"
            self._progress.update(self._alpha_task, status=status)
    
    def finish_batch(self, metrics: Dict = None):
        """Finish current batch (compatibility method)."""
        self.finish_alpha(metrics)
    
    def stop(self):
        """Stop progress tracking (compatibility method)."""
        self.finish()
