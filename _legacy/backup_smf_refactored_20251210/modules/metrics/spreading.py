...
def compute_qy_all_parallel(
    ...
    """
    Compute Q_Y over ALL edges (observed + unobserved) for all alphas.
    
    This is the TRUE Q_Y that measures generalization: cosine similarity
    between student Y and teacher Y over ALL N1*N2 entries (not just observed).
    
    Returns:
        Q_Y_all: (A,) cosine similarity over ALL edges
        Physical_Y_all: (A,) physical overlap <Y_s, Y_t> / <Y_t, Y_t> over ALL edges
    """
    from ..algorithms.bigamp_spreading_parallel import forward_pass_parallel
    ...
    Q_Y_all = torch.zeros(A, device=device)
    Physical_Y_all = torch.zeros(A, device=device)
    
    for a in range(A):
        # Use ALL C_max edges
        y_t = Y_teacher[:C_max]
        y_s = Y_student[a, :C_max]
        
        # Cosine Similarity
        dot = (y_t * y_s).sum()
        norm_t = y_t.norm()
        norm_s = y_s.norm()
        Q_Y_all[a] = dot / (norm_t * norm_s + 1e-12)
        
        # Physical Overlap
        Physical_Y_all[a] = dot / (norm_t * norm_t + 1e-12)
    
    return Q_Y_all, Physical_Y_all
...
def compute_all_metrics_spreading_parallel(
    ...
    """
    from .overlap import compute_cosine_similarity, gram_overlap_normalized, compute_physical_overlap
    ...
    Physical_Y_obs_all = torch.zeros(S, A, device=device)
    Physical_Y_unobs_all = torch.zeros(S, A, device=device)
    ...
    Physical_W_all = torch.zeros(S, A, device=device)
    Physical_X_all = torch.zeros(S, A, device=device)
    ...
    for s in range(S):
        ...
        # Physical Overlap Y (on observed edges only - legacy)
        ...
        # Q_W, Q_X, Q_W_prime, Q_X_prime and physical_overlap_W/X for each alpha
        for a in range(A):
            ...
            # Physical Overlap for W and X (absolute due to sign ambiguity)
            Physical_W_all[s, a] = compute_physical_overlap(
                W_students[s, a], W_teacher, absolute=True
            )
            Physical_X_all[s, a] = compute_physical_overlap(
                X_students[s, a], X_teacher, absolute=True
            )

    # Q_Y_all (TRUE generalization metric: ALL edges)
    Q_Y_all_all = torch.zeros(S, A, device=device)
    Physical_Y_global_all = torch.zeros(S, A, device=device)
    for s in range(S):
        Q_Y_all_all[s], Physical_Y_global_all[s] = compute_qy_all_parallel(
            W_students[s], X_students[s], spreading_data, s
        )
    ...
    results = {
        # Q_Y variants
        'Q_Y_mean': Q_Y_all_all.mean(dim=0),  # TRUE Q_Y over ALL edges
        'Q_Y_std': Q_Y_all_all.std(dim=0),
        'Q_Y_observed_mean': Q_Y_obs_all.mean(dim=0),
        'Q_Y_observed_std': Q_Y_obs_all.std(dim=0),
        'Q_Y_unobserved_mean': Q_Y_unobs_all.mean(dim=0),
        'Q_Y_unobserved_std': Q_Y_unobs_all.std(dim=0),
        # Physical Overlap Y
        'physical_overlap_Y_mean': Physical_Y_global_all.mean(dim=0),  # Over ALL edges
        'physical_overlap_Y_std': Physical_Y_global_all.std(dim=0),
        'physical_overlap_Y_observed_mean': Physical_Y_all.mean(dim=0),  # On observed edges
        'physical_overlap_Y_observed_std': Physical_Y_all.std(dim=0),
