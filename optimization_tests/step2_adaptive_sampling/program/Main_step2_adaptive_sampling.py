# ============================================================
# Teacher–Student Masked MF - STEP 3: Adaptive Sampling (Intelligent Alpha Selection)
# Perfect Bi-Regular Graph (no duplicate edges)
#
# STEP 3 MODIFICATION:
# - Two-stage training: coarse scan + intelligent fine sampling
# - Use PhaseTransitionAnalyzer to detect phase transition regions
# - Dense sampling in phase transition regions, sparse in stable regions
# - Dramatically reduce total training time while maintaining accuracy
#
# STEP 1: Manual Adam Optimizer (completed, ~10x speedup)
# STEP 2: Learning Rate Scheduling (completed, total ~20x speedup)
# STEP 3: Adaptive Sampling (current, total ~30x speedup expected)
#
# Original Optimizations (unchanged):
...
# Import PhaseTransitionAnalyzer
...
LR_SCHEDULER_ETA_MIN = 1e-6  # Minimum learning rate for cosine annealing

# ============================================================
# Adaptive Sampling Configuration
# ============================================================
USE_ADAPTIVE_SAMPLING = True  # Enable intelligent alpha sampling
COARSE_EPOCHS = 2000  # Coarse scan epochs (QUICK TEST: 200, FULL: 2000)
COARSE_STEP = 0.1  # Coarse scan alpha step
PHASE_STEP = 0.02  # Dense sampling in phase transition region
STABLE_STEP = 0.2  # Sparse sampling in stable regions
FINE_EPOCHS = 20000  # Fine training epochs (QUICK TEST: 500, FULL: 20000)
SENSITIVITY = 'medium'  # Anomaly detection: 'strict', 'medium', 'loose'
...
def train_batched_trials_agd(
    ...
    *,
    ...
    # ============================================================
    # Learning Rate Scheduler Function (Cosine Annealing)
    # ============================================================
    def get_cosine_annealing_lr(current_step, total_steps, lr_max, lr_min):
        """
        ...
        Args:
            ...
            lr_min: Minimum learning rate (eta_min)
        ...
        return lr_min + (lr_max - lr_min) * cos_factor

    # ============================================================
    # Training loop - Use BF16 mixed precision + Manual Adam + LR Scheduling
    # ============================================================
    ...
    for step in range(max_training_steps):
        ...
        current_lr = get_cosine_annealing_lr(step, max_training_steps, lr, LR_SCHEDULER_ETA_MIN)
        ...
        with torch.autocast(device_type=device.type, dtype=COMPUTE_DTYPE, enabled=USE_BF16):
            # --- 1) Compute gradient for W using current (W, X) ---
        ...
        # Adam update for W (outside autocast, use FP32 precision)
        grad_W = grad_W.float()  # Convert to FP32
        ...
        m_hat_W = m_W / (1 - beta1 ** (step + 1))

        # Compute bias-corrected second raw moment estimate
        v_hat_W = v_W / (1 - beta2 ** (step + 1))

        # Update W (use dynamic learning rate from scheduler)
        W = W - current_lr * m_hat_W / (torch.sqrt(v_hat_W) + eps)

        with torch.autocast(device_type=device.type, dtype=COMPUTE_DTYPE, enabled=USE_BF16):
            # --- 2) Recompute with updated W, compute gradient for X ---
        ...
        # Adam update for X (outside autocast, use FP32 precision)
        grad_X = grad_X.float()  # Convert to FP32
        ...
        # Update X (use dynamic learning rate from scheduler)
        X = X - current_lr * m_hat_X / (torch.sqrt(v_hat_X) + eps)
...
# ------------------------------------------------------------
# Main - Adaptive Sampling Integration
# ------------------------------------------------------------
if __name__ == "__main__":
    print("\n" + "=" * 100)
    print("STEP 3: ADAPTIVE SAMPLING - Intelligent Alpha Selection")
    print("=" * 100)

    if USE_ADAPTIVE_SAMPLING:
        print("\n[Mode] Adaptive Sampling ENABLED")
        print(f"  Coarse scan: Δα={COARSE_STEP}, epochs={COARSE_EPOCHS}")
        print(f"  Fine training: Phase Δα={PHASE_STEP}, Stable Δα={STABLE_STEP}, epochs={FINE_EPOCHS}")
        print(f"  Sensitivity: {SENSITIVITY}")

        # Initialize teacher model once
        set_seed(SEED)
        Wt, Xt = create_teacher_dense(N1, N2, M, DEVICE, seed=SEED)

        # Define training callback that wraps train_all_alphas_parallel
        def training_callback(alphas_array, epochs):
            """
            Training callback for PhaseTransitionAnalyzer

            Args:
                alphas_array: numpy array of alpha values
                epochs: number of training epochs

            Returns:
                metrics_dict: {str(alpha): {metric_name: value, ...}}
            """
            print(f"\n  Training {len(alphas_array)} alphas for {epochs} epochs...")

            # Convert numpy array to list for train_all_alphas_parallel
            alpha_list = alphas_array.tolist() if isinstance(alphas_array, np.ndarray) else list(alphas_array)

            # Call existing parallel training function
            results = train_all_alphas_parallel(
                Wt, Xt, alpha_list,
                steps=epochs,
                S=SAMPLES_PER_ALPHA,
            ...
            # Convert float keys to string keys for PhaseTransitionAnalyzer
            results_str_keys = {str(k): v for k, v in results.items()}
            return results_str_keys

        # Run adaptive analysis
        adaptive_results = PhaseTransitionAnalyzer.simple_adaptive_analysis(
            train_callback=training_callback,
            alpha_range=(ALPHA_TILDE_START, ALPHA_TILDE_STOP),
            ...
            save_smart_alphas=RESULT_DIR / 'smart_alphas.npy',
            verbose=True
        )

        # Extract final results for plotting
        results = adaptive_results['fine_results']
        smart_alphas = adaptive_results['smart_alphas']

        # Save analysis results
        analysis_json_path = RESULT_DIR / 'adaptive_analysis.json'
        with open(analysis_json_path, 'w') as f:
            json.dump({
                'phase_analysis': adaptive_results['phase_analysis'],
                'adaptive_plan': adaptive_results['adaptive_plan'],
                'speedup': adaptive_results['speedup'],
                'smart_alphas': smart_alphas
            }, f, indent=2)
        print(f"\n[Adaptive analysis saved] {analysis_json_path}")

        # Generate phase transition report using coarse results
        print("\n" + "=" * 80)
        print("PHASE TRANSITION ANALYSIS REPORT")
        print("=" * 80)

        coarse_alphas = adaptive_results['coarse_alphas']
        coarse_results = adaptive_results['coarse_results']

        analyzer = PhaseTransitionAnalyzer(
            coarse_alphas,
            coarse_results,
            severity_threshold=2.0 if SENSITIVITY == 'medium' else (3.0 if SENSITIVITY == 'strict' else 0.5)
        )
        ...
        plot_path = RESULT_DIR / 'phase_transition_analysis.png'
        analyzer.plot_full_analysis(save_path=plot_path, show=False)
        print(f"[Analysis plot saved] {plot_path}")

    else:
        print("\n[Mode] Adaptive Sampling DISABLED - Using direct training")
        # Fallback to original direct training
    ...
    if USE_ADAPTIVE_SAMPLING:
        json_filename = f'results_step3_adaptive_coarse{COARSE_EPOCHS}_fine{FINE_EPOCHS}.json'
    else:
        json_filename = f'results_step3_direct_epoch{EPOCHS_PER_ALPHA}.json'
