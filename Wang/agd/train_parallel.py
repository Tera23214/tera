    ...
    *,
    ...
    with torch.no_grad():
        ...
        for s in range(S):
            ...
            # A_b shape: (S, N1, N2) if batched, (1, N1, N2) if shared
            mask = A_b[s] if A_b.shape[0] == S else A_b[0]  # (N1, N2)
            unobs_mask = 1.0 - mask  # observed=0, unobserved=1
            Y_unobs = Yt * unobs_mask
            ...
            results.append(dict(
                ...
                Q_Y=float(Q_Y),
                Q_Y_unobserved=float(Q_Y_unobserved),
        ...
        for alpha_idx, alpha_tilde in enumerate(alpha_values):
            ...
            for s in range(S):
                ...
                trial_results.append({
                    ...
                    'Q_Y': float(Q_Y), 'Q_Y_unobserved': float(Q_Y_unobserved),
                    'Gen_Error': float(gen_error),
            ...
            qY_unobs = [s['Q_Y_unobserved'] for s in trial_results]
            ...
            QY_unobs_mean, QY_unobs_std = mean_std(qY_unobs)
            ...
            results[float(alpha_tilde)] = {
                ...
                'Q_Y_unobserved_mean': QY_unobs_mean, 'Q_Y_unobserved_std': QY_unobs_std,
    ...
    for aT in pbar:
        ...
        if C == 0:
            results[float(aT)] = dict(
                ...
                Q_Y_unobserved_mean=0.0, Q_Y_unobserved_std=0.0,
        ...
        qY_unobs = [s['Q_Y_unobserved'] for s in trial_stats]
...
def display_results(results_dict):
    ...
    for _, r in items:
        rows.append({
            ...
            'Q_Y_unobs': f"{r.get('Q_Y_unobserved_mean', 0):.4f}±{r.get('Q_Y_unobserved_std', 0):.4f}",
...
def plot_results(results_dict):
    ...
    qY_unobs_mu = np.array([kv[1].get('Q_Y_unobserved_mean', 0) for kv in items])
    qY_unobs_sd = np.array([kv[1].get('Q_Y_unobserved_std', 0) for kv in items])
    ...
    # ============================================================
    # Q_Y vs Q_Y_unobserved comparison chart (generalization)
    # ============================================================
    fig_qy_cmp = plt.figure(figsize=(10, 7))
    ax_qy = fig_qy_cmp.add_subplot(111)

    # Q_Y (full matrix)
    ax_qy.plot(aL, qY_mu, marker='D', linewidth=2, markersize=6,
               color='#d62728', label='Q_Y (full matrix)', zorder=3)
    if np.any(qY_sd > 0):
        ax_qy.fill_between(aL, qY_mu - qY_sd, qY_mu + qY_sd,
                           color='#d62728', alpha=0.15)

    # Q_Y_unobserved (generalization)
    ax_qy.plot(aL, qY_unobs_mu, marker='o', linewidth=2, markersize=6,
               color='#1f77b4', label='Q_Y_unobserved (generalization)', zorder=2)
    if np.any(qY_unobs_sd > 0):
        ax_qy.fill_between(aL, qY_unobs_mu - qY_unobs_sd, qY_unobs_mu + qY_unobs_sd,
                           color='#1f77b4', alpha=0.15)

    ax_qy.set_xlabel(r'$\tilde{\alpha}_L = C / (M \cdot N_1)$', fontsize=13)
    ax_qy.set_ylabel('Overlap', fontsize=13)
    ax_qy.set_title('Q_Y Comparison: Full Matrix vs Unobserved (Generalization)', fontsize=14, fontweight='bold')
    ax_qy.set_ylim(-0.05, 1.05)
    ax_qy.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
    ax_qy.legend(fontsize=11, loc='lower right')

    plt.tight_layout()
    qy_cmp_filename = f"{graph_type}_{resample}_{early_stop_flag}_{key_param}_batch{SAMPLES_PER_ALPHA}_qy_generalization.png"
    qy_cmp_path = RESULT_DIR / qy_cmp_filename
    fig_qy_cmp.savefig(qy_cmp_path, dpi=300, bbox_inches='tight')
    print(f"Q_Y generalization comparison saved as: {qy_cmp_path}")
    plt.close(fig_qy_cmp)
...
if __name__ == "__main__":
    ...
    # Save results to JSON (for comparison with BiG-AMP)
    json_filename = f"agd_results_epochs{EPOCHS_PER_ALPHA}.json"
